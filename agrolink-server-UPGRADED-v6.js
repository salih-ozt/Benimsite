...
// Ana sayfa gönderilerini getir (Karışık: her 5 mavi tikli sonrası 2 mavi tiksiz)
app.get('/api/posts', authenticateToken, async (req, res) => {
    try {
        ...
        // Mavi tikli kullanıcıların postları (son 5 gün)
        const verifiedPosts = await db.all(
            `SELECT 
                p.*,
                p.likeCount,
                p.commentCount,
        ...
        // Mavi tiksiz kullanıcıların postları (son 5 gün)
        const unverifiedPosts = await db.all(
            `SELECT 
                p.*,
                p.likeCount,
                p.commentCount,
        ...
        while (mixedPosts.length < limitNum && (vIndex < verifiedPosts.length || uIndex < unverifiedPosts.length)) {
            // 5 mavi tikli ekle
            for (let i = 0; i < 5 && vIndex < verifiedPosts.length && mixedPosts.length < limitNum; i++) {
                mixedPosts.push(verifiedPosts[vIndex++]);
            }
            // 2 mavi tiksiz ekle
            for (let i = 0; i < 2 && uIndex < unverifiedPosts.length && mixedPosts.length < limitNum; i++) {
                mixedPosts.push(unverifiedPosts[uIndex++]);
            }
        }

        // Eğer mavi tikli bittiyse kalan mavi tiksizleri ekle
        while (mixedPosts.length < limitNum && uIndex < unverifiedPosts.length) {
            mixedPosts.push(unverifiedPosts[uIndex++]);
        }
        // Eğer mavi tiksiz bittiyse kalan mavi tiklileri ekle
        while (mixedPosts.length < limitNum && vIndex < verifiedPosts.length) {
            mixedPosts.push(verifiedPosts[vIndex++]);
        }
