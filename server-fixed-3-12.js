...
async function createNotification(userId, type, message, data = {}) {
    try {
        ...
        const pushIcons = { like: '❤️', comment: '💬', follow: '👥', message: '📩', story_like: '⭐', comment_like: '👍', mention: '📢', new_post: '📸', tag_request: '🏷️', tag_rejected: '❌', student_apply: '🎓', student_approved: '🎓', student_rejected: '❌' };
        ...
        } else if (type === 'student_apply') {
            pushTitle = '🎓 Yeni Öğrenci Başvurusu';
            pushBody  = message;
        } else if (type === 'student_approved') {
            pushTitle = '🎓 Başvurunuz Onaylandı!';
            pushBody  = 'Tarım Lisesi Öğrencisi doğrulamanız onaylandı. Premium üyeliğiniz aktif!';
        } else if (type === 'student_rejected') {
            pushTitle = '❌ Başvurunuz Reddedildi';
            pushBody  = message;
        }
...
function verifyCsrf(req, res, next) {
    ...
    const csrfExempt = [
        ...
        '/api/auth/reset-password-with-token', // Kendi resetToken'ı ile korunuyor + CSRF cookie inject ediliyor
...
app.post('/api/auth/register', registerLimiter, validateAuthInput, upload.single('profilePic'), async (req, res) => {
    try {
        ...
        const validUserTypes = ['tarim_ogretmeni', 'tarim_ogrencisi', 'tarim_lisesi_ogrencisi', 'ogretmen', 'ziraat_muhendisi', 'normal_kullanici', 'ciftci_hayvancilik'];
...
// ─── 11. FEED ───────────────────────────────────────────────────────
// 🌾 AGROLINK FEED v5 — Sunucu Taraflı Seen Takibi
//
// Frontend sadece GET /api/feed?refresh=1 atar (refresh=1 görülenleri sıfırlar)
// Sunucu her kullanıcının gördüğü postları bellekte tutar
// Aynı post hiç tekrar gelmez — frontend hiçbir şey yapmaz

// Kullanıcı başına görülen post ID seti (bellek içi, process restart'ta sıfırlanır)
// Map<userId, { ids: Set<string>, lastAccess: number }>
const feedSeenStore = new Map();
const FEED_SEEN_TTL = 2 * 60 * 60 * 1000; // 2 saat sonra sıfırla (yeni içerik görsün)

// Eski kayıtları temizle (saatte bir)
setInterval(() => {
    const now = Date.now();
    for (const [uid, data] of feedSeenStore.entries()) {
        if (now - data.lastAccess > FEED_SEEN_TTL) feedSeenStore.delete(uid);
    }
}, 60 * 60 * 1000);

function getFeedSeen(userId) {
    const entry = feedSeenStore.get(userId);
    if (!entry) return new Set();
    if (Date.now() - entry.lastAccess > FEED_SEEN_TTL) {
        feedSeenStore.delete(userId);
        return new Set();
    }
    return entry.ids;
}

function addFeedSeen(userId, ids) {
    let entry = feedSeenStore.get(userId);
    if (!entry) entry = { ids: new Set(), lastAccess: Date.now() };
    for (const id of ids) entry.ids.add(id);
    entry.lastAccess = Date.now();
    // Max 2000 post tut (bellek taşmasın)
    if (entry.ids.size > 2000) {
        const arr = [...entry.ids];
        entry.ids = new Set(arr.slice(arr.length - 1000));
    }
    feedSeenStore.set(userId, entry);
}

function interleaveByUser(posts) {
    const queues = new Map();
    const order  = [];
    for (const p of posts) {
        const uid = p.userId || p.user_id;
        if (!queues.has(uid)) { queues.set(uid, []); order.push(uid); }
        queues.get(uid).push(p);
    }
    const result = [];
    let lastUid  = null;
    while (true) {
        const alive = order.filter(uid => queues.has(uid) && queues.get(uid).length > 0);
        if (alive.length === 0) break;
        let pick = alive.find(uid => uid !== lastUid) || alive[0];
        const q  = queues.get(pick);
        result.push(q.shift());
        if (q.length === 0) queues.delete(pick);
        lastUid = pick;
    }
    return result;
}
...
app.get('/api/feed', authenticateToken, async (req, res) => {
    try {
        ...
        const refresh = req.query.refresh === '1'; // feed yenile → görülenleri sıfırla

        // refresh=1 ise seen listesini sıfırla (pull-to-refresh)
        if (refresh) feedSeenStore.delete(req.user.id);

        const seenIds  = [...getFeedSeen(req.user.id)];
        const seenUUIDs = seenIds.filter(s => /^[0-9a-f-]{36}$/i.test(s));

        // ── A) Takip edilenlerin postları ────────────────────────────────
        const followedParams  = [req.user.id];
        let   followedSeenSQL = '';
        if (seenUUIDs.length > 0) {
            followedSeenSQL = `AND p.id NOT IN (${seenUUIDs.map((_, i) => `$${i + 2}`).join(',')})`;
            followedParams.push(...seenUUIDs);
        }

        const followedRaw = await dbAll(
            `SELECT
                p.id, p."userId", p.content, p.media, p."mediaType", p."mediaUrls",
                p."createdAt", p."likeCount", p."commentCount", p."allowComments",
                p."locationName", p.latitude, p.longitude,
                p."isPoll", p."pollQuestion", p."pollOptions",
                u.name, u."profilePic", u."isVerified", u."hasFarmerBadge",
                u."userType", u."farmerBadgeType", u."farmerCertificate",
                ...
                TRUE AS "isFollowing",
                EXTRACT(EPOCH FROM p."createdAt") AS _ts
             FROM posts p
             JOIN users u ON p."userId" = u.id
             JOIN follows f ON f."followingId" = p."userId" AND f."followerId" = $1
        ...
        // Kişi başı max 3 post
        const followedCapped = [];
        const countPerUser   = new Map();
        for (const p of followedRaw) {
            const c = countPerUser.get(p.userId) || 0;
            if (c < 3) { followedCapped.push(p); countPerUser.set(p.userId, c + 1); }
        }

        // ── B) Keşfet postları ───────────────────────────────────────────
        const followedUserIds = [...new Set(followedRaw.map(p => p.userId))];
        const discoverTarget  = Math.max(Math.ceil(lim * 0.3), 5);
        const discoverParams  = [req.user.id, discoverTarget * 3];
        let   discoverSQL     = '';

        if (followedUserIds.length > 0) {
            const s = discoverParams.length + 1;
            discoverSQL += ` AND p."userId" NOT IN (${followedUserIds.map((_, i) => `$${s + i}`).join(',')})`;
            discoverParams.push(...followedUserIds);
        }
        if (seenUUIDs.length > 0) {
            const s = discoverParams.length + 1;
            discoverSQL += ` AND p.id NOT IN (${seenUUIDs.map((_, i) => `$${s + i}`).join(',')})`;
            discoverParams.push(...seenUUIDs);
        }

        const discoverPosts = await dbAll(
            `SELECT
                p.id, p."userId", p.content, p.media, p."mediaType", p."mediaUrls",
                p."createdAt", p."likeCount", p."commentCount", p."allowComments",
                p."locationName", p.latitude, p.longitude,
             ...
             LIMIT $2`,
            discoverParams
        ).catch(e => { console.error('[Feed] Keşfet hatası:', e.message); return []; });

        // ── C) Karıştır: 7 takip + 3 keşfet blok ────────────────────────
        const sortByTime     = arr => [...arr].sort((a, b) => (b._ts || 0) - (a._ts || 0));
        const sortedFollowed = sortByTime(followedCapped);
        const sortedDiscover = sortByTime(discoverPosts);

        const merged = [];
        let fi = 0, di = 0;
        while (fi < sortedFollowed.length || di < sortedDiscover.length) {
            for (let k = 0; k < 7 && fi < sortedFollowed.length; k++, fi++) merged.push(sortedFollowed[fi]);
            for (let k = 0; k < 3 && di < sortedDiscover.length; k++, di++) merged.push(sortedDiscover[di]);
            if (merged.length >= lim * 3) break;
        }

        // ── D) Interleave + limit ─────────────────────────────────────────
        const interleaved = interleaveByUser(merged);
        const feedPosts   = interleaved.slice(0, lim);

        // ── E) Görülenleri sunucuda kaydet ───────────────────────────────
        addFeedSeen(req.user.id, feedPosts.map(p => p.id));

        const totalSeen = getFeedSeen(req.user.id).size;
        console.log(`[Feed v5] userId=${req.user.id} takip=${followedCapped.length} keşfet=${discoverPosts.length} dönen=${feedPosts.length} toplamGörülen=${totalSeen} refresh=${refresh}`);

        res.json({
            posts    : feedPosts.map(formatPost),
            hasMore  : interleaved.length > lim || followedRaw.length === 200,
            algorithm: 'server-seen-v5',
        });
    } catch (error) {
        console.error('[Feed] Hata:', error.message, error.detail || '');
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.get('/api/auth/reset-password-direct', async (req, res) => {
    ...
    try {
        ...
        // 🔒 CSRF cookie'si yoksa yeni üret ve set et
        // (Kullanıcı henüz giriş yapmamış olabilir, bu sayfada CSRF olmadan form gönderilemez)
        if (!req.cookies?.csrf_token) {
            const freshCsrf = generateCsrfToken();
            setCsrfCookie(res, req, freshCsrf);
        }
...
// ════════════════════════════════════════════════════════════════════
// 🎓 TARIM LİSESİ ÖĞRENCİSİ DOĞRULAMA SİSTEMİ
// Öğrenci başvuru yapar → Admin (agro_sosyal) onaylar → Premium açılır
// ════════════════════════════════════════════════════════════════════

// POST /api/student/apply — Öğrenci başvurusu gönder
app.post('/api/student/apply', authenticateToken, uploadLimiter, upload.single('document'), async (req, res) => {
    try {
        const { schoolName, studentNumber, schoolEmail } = req.body;
        if (!schoolName || !studentNumber) return res.status(400).json({ error: 'Okul adı ve öğrenci numarası zorunludur' });

        // Daha önce başvuru yapmış mı?
        const existing = await dbGet(
            `SELECT id, status FROM student_verifications WHERE "userId"=$1 ORDER BY "createdAt" DESC LIMIT 1`,
            [req.user.id]
        );
        if (existing) {
            if (existing.status === 'pending')  return res.status(409).json({ error: 'Bekleyen bir başvurunuz zaten var' });
            if (existing.status === 'approved') return res.status(409).json({ error: 'Zaten onaylanmış öğrenci hesabınız var' });
        }

        // Belge yükle (opsiyonel ama önerilir)
        let documentUrl = null;
        if (req.file) {
            try {
                const filename = `student_doc_${uuidv4().replace(/-/g,'').slice(0,16)}.webp`;
                const destPath = path.join(postsDir, filename);
                await processImage(req.file.path, destPath, { width: 1600, height: 1600, fit: 'inside', quality: 80, effort: 1 });
                await fs.unlink(req.file.path).catch(() => {});
                documentUrl = `/uploads/posts/${filename}`;
            } catch { await fs.unlink(req.file?.path || '').catch(() => {}); }
        }

        const applyId = uuidv4();
        await dbRun(
            `INSERT INTO student_verifications
             (id, "userId", "schoolName", "studentNumber", "schoolEmail", "documentUrl", status, "createdAt")
             VALUES ($1,$2,$3,$4,$5,$6,'pending',NOW())`,
            [applyId, req.user.id, schoolName.trim(), studentNumber.trim(), schoolEmail?.trim() || null, documentUrl]
        );

        // Admin (agro_sosyal) kullanıcısına bildirim gönder
        const admin = await dbGet(`SELECT id FROM users WHERE username='agro_sosyal' LIMIT 1`);
        if (admin) {
            const applicant = await dbGet(`SELECT name, username FROM users WHERE id=$1`, [req.user.id]);
            createNotification(
                admin.id,
                'student_apply',
                `🎓 ${applicant?.name || applicant?.username} tarım lisesi öğrenci doğrulaması için başvurdu`,
                { applyId, applicantId: req.user.id, schoolName, studentNumber }
            ).catch(() => {});
        }

        console.log(`[StudentApply] userId=${req.user.id} okul="${schoolName}" no="${studentNumber}"`);
        res.status(201).json({ success: true, message: 'Başvurunuz alındı. Admin onayı bekleniyor.' });
    } catch (e) {
        console.error('[StudentApply]', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// GET /api/student/status — Kendi başvuru durumunu gör
app.get('/api/student/status', authenticateToken, async (req, res) => {
    try {
        const record = await dbGet(
            `SELECT id, "schoolName", "studentNumber", "schoolEmail", status, "createdAt", "reviewedAt", "reviewNote"
             FROM student_verifications WHERE "userId"=$1 ORDER BY "createdAt" DESC LIMIT 1`,
            [req.user.id]
        );
        res.json({ application: record || null });
    } catch (e) {
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// GET /api/admin/student-applications — Admin: bekleyen başvuruları listele
app.get('/api/admin/student-applications', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { status = 'pending', page = 1 } = req.query;
        const lim = 20, off = (parseInt(page) - 1) * lim;
        const apps = await dbAll(
            `SELECT sv.*, u.name, u.username, u."profilePic", u.email
        ...
        const total = await dbGet(`SELECT COUNT(*) AS c FROM student_verifications WHERE status=$1`, [status]);
        res.json({ applications: apps.map(a => ({ ...a, documentUrl: absoluteUrl(a.documentUrl) })), total: parseInt(total?.c || 0) });
    } catch (e) {
        console.error('[StudentApps]', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

// POST /api/admin/student-applications/:id/review — Admin onay/red
// Body: { action: 'approve' | 'reject', note?: string }
app.post('/api/admin/student-applications/:id/review', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { action, note } = req.body;
        if (!['approve', 'reject'].includes(action)) return res.status(400).json({ error: 'action approve veya reject olmalı' });

        const appl = await dbGet(`SELECT * FROM student_verifications WHERE id=$1`, [id]);
        if (!appl) return res.status(404).json({ error: 'Başvuru bulunamadı' });
        if (appl.status !== 'pending') return res.status(409).json({ error: 'Bu başvuru zaten işlendi' });

        const newStatus = action === 'approve' ? 'approved' : 'rejected';
        await dbRun(
            `UPDATE student_verifications SET status=$1, "reviewedAt"=NOW(), "reviewNote"=$2 WHERE id=$3`,
            [newStatus, note || null, id]
        );

        if (action === 'approve') {
            // userType → tarim_lisesi_ogrencisi, plan → premium, hasFarmerBadge → true
            await dbRun(
                `UPDATE users SET
                    "userType"='tarim_lisesi_ogrencisi',
                    plan='premium',
                    "hasFarmerBadge"=TRUE,
                    "farmerBadgeType"='tarim_lisesi_ogrencisi',
                    "updatedAt"=NOW()
                 WHERE id=$1`,
                [appl.userId]
            );
            ...
            createNotification(
                appl.userId,
                'student_approved',
                '🎓 Tarım Lisesi Öğrencisi doğrulamanız onaylandı! Premium üyeliğiniz aktif edildi.',
                { schoolName: appl.schoolName }
            ).catch(() => {});
            console.log(`[StudentApprove] userId=${appl.userId} okul="${appl.schoolName}" → premium açıldı`);
        } else {
            // Reddedildiyse öğrenciye bildirim
            createNotification(
                appl.userId,
                'student_rejected',
                `❌ Tarım Lisesi Öğrencisi başvurunuz reddedildi.${note ? ' Sebep: ' + note : ''}`,
                { note }
            ).catch(() => {});
            console.log(`[StudentReject] userId=${appl.userId} sebep="${note}"`);
        }

        res.json({ success: true, status: newStatus });
    } catch (e) {
        console.error('[StudentReview]', e.message);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
// ── 🎓 Tarım Lisesi Öğrencisi Doğrulama Tablosu ──────────────────────
(async () => {
    try {
        await pool.query(`
        ...
        `);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_sv_user   ON student_verifications("userId")`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_sv_status ON student_verifications(status)`);
        // validUserTypes'a tarim_lisesi_ogrencisi ekle (DB'de constraint yoksa sadece uygulama seviyesinde)
        console.log('✅ [StudentVerification] Tablo hazır');
    } catch (e) {
        console.warn('[StudentVerification] Migration:', e.message);
    }
})();
